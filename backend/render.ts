...
function renderPlainInner(inv: Investigation): string {
  ...
  if (lead?.segment) {
    ...
    const share = lead.segmentSharePct != null ? ` They are ${asFraction(lead.segmentSharePct)}.` : "";
    ...
    const moveSentence = ba
      ...
      : lead.deltaPct != null
        ...
        : "";
    L.push(`  ${wrap(`${seg.dimension.replace(/_/g, " ")} '${seg.value}' — ${moveSentence}${share}`, 2)}`);
    if (segmentFindings.length > 1) {
      L.push(`  ${wrap("(A few other segments moved too; see the full trace for all of them.)", 2)}`);
    }
    L.push("");
  } else if (inv.primaryChannel === "not_localizable") {
    L.push("WHY");
    L.push(`  ${wrap("Every part of the platform moved together, by about the same amount. Nothing is localized to one place.", 2)}`);
    L.push("");
  }
  ...
  const cleared = inv.ruledOut.filter(
    (r) => !r.segment && (r.status === "cleared_as_normal" || r.status === "cleared_insufficient_data"),
  );
