...
| ID | Task | Lane | Owner | Status | Branch | Depends on | Notes |
|---|---|---|---|---|---|---|---|
| T-013 | Rollup table + MV from `ad_events` (sums only, never pre-averaged ratios) | Lane B | — | todo | — | T-006 | **CRITICAL PATH under D-019 — this is the scalability story, not an optimisation.** Every investigation today full-scans 9.00M rows / 77.71 MiB / 216 MiB peak / ~1.2s per sweep. At `(hour × dimension × value)` grain this window is ~59k rows, and the rollup grows with cardinality × time rather than with events — so the same query costs the same at 9M events or 9T. Per `metrics_glossary.md`, sums only; ratios stay sum/sum at query time. |
