...
import { DATABASE, exec, makeClient, select, selectOne } from "../clickhouse/client";
import {
  CHUNK_DATE_PATTERN,
  CHUNK_DIR,
  CHUNK_GLOB,
...
async function loadDimensions(client: ClickHouseClient): Promise<void> {
  ...
  for (const { table, file } of DIMENSION_SOURCES) {
    ...
    await withRetry(`load ${table}`, RETRY_ATTEMPTS, async () => {
      ...
      await client.insert({
        table,
        values: createReadStream(path),
        format: DataFormat.CsvWithNames,
      });
    });
...
async function extractChunks(options: LoadOptions): Promise<DayChunk[]> {
  ...
  if (options.skipExtract) {
    console.log("== extract (skipped, reusing existing chunks) ==");
  } else {
    console.log("== extract ==");
    const startedAt = performance.now();
    ...
    console.log(`  split ${relPath(FACT_FILE)} by day in ${secondsSince(startedAt)}`);
  }
  ...
  const chunks: DayChunk[] = meta.map(({ file_name, num_rows }) => {
    ...
    const date = match[1]!;
    return { date, partition: date.replaceAll("-", ""), path: file_name, rows: Number(num_rows) };
  });
...
async function partitionCounts(client: ClickHouseClient): Promise<Map<string, number>> {
...
function selectPending(
  ...
  return chunks.filter((chunk) => {
    ...
    if (loaded === chunk.rows) {
      console.log(`  ${chunk.date}  skip (already ${fmt(loaded)} rows)`);
      return false;
    }
    return true;
  });
...
async function loadDay(client: ClickHouseClient, chunk: DayChunk): Promise<void> {
  ...
  await withRetry(`load ${chunk.date}`, RETRY_ATTEMPTS, async () => {
    ...
    await exec(client, Q.dropPartition(chunk.partition), { alter_sync: "2" });

    await client.insert({
      table: Table.AdEvents,
      values: createReadStream(chunk.path),
...
async function loadFacts(client: ClickHouseClient, options: LoadOptions): Promise<void> {
  await assertDuckdb();
  ...
  console.log(`== load ${Table.AdEvents} ==`);
  const pending = selectPending(chunks, await partitionCounts(client), options);

  if (pending.length === 0) {
    console.log("  nothing to do -- all days already match the source\n");
    return;
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(`Done in ${secondsSince(startedAt)}. Next: bun run ch:verify`);
  } finally {
    await client.close();
  }
}
