...
function rankAndGroup(windows: SweepWindow[], avgRequestsPerDay: number): JoinedIncident[] {
  const scored = windows
    .map((w) => {
      const leadSharePct = avgRequestsPerDay > 0 ? (w.requestsPerDay / avgRequestsPerDay) * 100 : null;
      /**
  ...
  const sameWindow = (a: SweepWindow, b: SweepWindow): boolean => a.from === b.from && a.to === b.to;
...
async function main(): Promise<void> {
  ...
  const metrics = arg("metrics")?.split(",").map((m) => m.trim()).filter(Boolean);
  const from = arg("from");
  const to = arg("to");
  ...
  try {
    ...
    for (const j of selected) {
      ...
      const ruledOut = (d.ruledOut ?? []) as Array<{ status: string; note?: string; evidenceIds: string[] }>;
      ...
      const referenced = new Set(
        [...findings, ...ruledOut].flatMap((f) => f.evidenceIds ?? []).map((e) => `${trace.callId}/${e}`),
      );
      ...
      diagnosed.push({
        ...
        grounding,
        revenueUsdPerDay: findings.find((f) => f.revenueImpactUsd !== null)?.revenueImpactUsd ?? null,
        ruledOutContamination: ruledOut.filter((r) => r.status === "cleared_as_contamination").length,
    ...
    say("");
    say(`[diagnose] ${diagnosed.length} incident(s) diagnosed in ${((Date.now() - started) / 1000).toFixed(1)}s`);
