...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const p of parts) {
      ...
      say(`  ${p.name.padEnd(24)} ${Number(p.rows).toLocaleString().padStart(12)} rows  ${(Number(p.bytes) / 1048576).toFixed(1)} MiB`);
    }
    if (parts.length > 1) {
      say(`  ${"total".padEnd(24)} ${totalRows.toLocaleString().padStart(12)} rows  ${(totalBytes / 1048576).toFixed(1)} MiB`);
    }
    ...
    say(`[synth] gone. Rebuild any time with \`bun run synth:build\` — seed-fixed, so it comes back identical.`);
  } finally {
    await client.close();
  }
}
