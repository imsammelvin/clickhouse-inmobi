...
const GATES: Gate[] = [
  ...
  },
  {
    ...
    skipCode: 2,
    summary: (o) =>
      lastMatch(o, /gated failures\s+\d+/g) ?? lastMatch(o, /(STALE DATASET|SETUP ERROR|Refusing to run)[^\n]*/g),
  },
];
...
if (failed.length) {
  ...
  process.stdout.write(`Re-run one on its own for full output, e.g. \`${failed[0]!.gate.cmd.join(" ")}\`\n`);
}
process.stdout.write("\n");
