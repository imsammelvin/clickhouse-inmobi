...
import type { Channel, Investigation, Segment } from "../backend/types";
...
function whereToLook(
  ...
  switch (channel) {
    ...
    case "no_anomaly":
      return { lines: [`Nothing to do — the metric is within its normal band.`], nextQuestion: null };
  }
}
...
async function resolveStatus(
  ...
  if (inv.primaryChannel === "no_anomaly" || inv.primaryChannel === "seasonality") {
    return { status: "not_applicable", detail: "Nothing was wrong, so there is nothing to recover from.", daysRunning: null };
  }
  ...
  if (firstRecovered) {
    ...
    return {
      ...
      detail:
        ...
        (ran > 1 && lastBad ? `, ${affected[0]} to ${lastBad}` : ran === 1 ? `, ${affected[0]}` : "") +
        `.`,
      daysRunning: ran,
    };
  }
...
export async function recommendAction(
  ...
  const lost = perDay !== null && daysRunning !== null ? Number((perDay * daysRunning).toFixed(2)) : null;
  ...
  if (inv.primaryChannel === "no_anomaly" || inv.primaryChannel === "seasonality" || inv.primaryChannel === "mix_shift") {
    priority = "none";
...
const fmt = (n: number): string => (Math.abs(n) < 10 ? n.toFixed(4) : Math.round(n).toLocaleString("en-US"));
