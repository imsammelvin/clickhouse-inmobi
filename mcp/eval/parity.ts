...
import type { Evidence, Investigation } from "../../backend/types";
...
async function main(): Promise<void> {
  ...
  say(`  rollup: ${health.ready ? `ready (${health.factEvents.toLocaleString()} events)` : `NOT ready — ${health.reason}`}`);
  if (!health.ready) {
    say(``);
    say(`  Nothing to compare: with the rollup unavailable both passes read the raw view, so a pass`);
