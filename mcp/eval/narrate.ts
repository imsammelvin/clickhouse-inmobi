...
import type { Investigation } from "../../backend/types";
...
async function main(): Promise<void> {
  ...
  for (const c of CASES) {
    ...
    if (forbidden.length) say(`        FORBIDDEN  claims this data cannot support: ${forbidden.join(", ")}`);
    if (tooLong) say(`        TOO LONG   ${out.text.length} chars, contract asks for short (${c.maxChars} cap here)`);
    say(`        first line: ${out.text.split("\n").find((l) => l.trim())?.slice(0, 100) ?? "(empty)"}`);
  }
  ...
  try {
    ...
    const distinctive = ["Lead with the answer", "WHAT STAYS INTERNAL", "status icon", "revenue manager"];
