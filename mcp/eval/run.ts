...
const near = (actual: unknown, n: Near): boolean =>
  typeof actual === "number" && Number.isFinite(actual) && Math.abs(actual - n.value) <= n.tolerance;
...
async function runCase(session: Session, c: EvalCase): Promise<CaseResult> {
  ...
  if (c.kind === "refusal") {
    ...
    if (isError) {
      ...
      const grounding = (parsed.grounding ?? {}) as { ok?: boolean; grounded?: number; numeralsChecked?: number };
...
async function main(): Promise<void> {
  ...
  if (cases.length === 0) {
    process.stderr.write(`No case matching '${only}'. Known: ${CASES.map((c) => c.id).join(", ")}\n`);
    process.exit(2);
  }
  ...
  try {
    ...
    for (const c of cases) {
      ...
      results.push(r);
      if (!asJson) {
        out(`${r.passedGated ? "PASS" : "FAIL"}  ${r.id.padEnd(28)} ${r.ms.toString().padStart(6)}ms  ${r.question}`);
    ...
    const summary = {
      ...
      casesPassed,
      gated: { checks: gated.length, passed: gatedPassed, accuracy: pct(gatedPassed, gated.length) },
      reported: {
      ...
      trace: exported.path,
      failures: results.filter((r) => !r.passedGated).map((r) => ({
        id: r.id,
    ...
    if (asJson) {
      ...
      out(`  gated accuracy      ${gatedPassed}/${gated.length}  ${summary.gated.accuracy}   (localization, no-false-alarm, grounding, refusals)`);
      out(`  reported accuracy   ${reportedPassed}/${reported.length}  ${summary.reported.accuracy}   (cause channel, dollars — measured, not gating)`);
