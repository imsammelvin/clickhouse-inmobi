...
|---|---|---|---|---|
| Ingest & schema | Parquet/CSV → ClickHouse; DDL, dictionaries, MV rollups, baseline views; idempotent reload | ClickHouse, `clickhouse-client` | Lane A | `ingest/` |
| Analysis engine | Every detector and drill-down as parameterised SQL; emits `Finding[]` + `Evidence[]` | ClickHouse SQL + thin Python | Lane B | `analysis/` |
| API & orchestration | Investigation-plan runner, evidence store, HTTP surface, Langfuse tracing | Python + FastAPI | Lane C | `api/` |
...
Shared root files (`docker-compose.yml`, `.gitignore`, `README.md`) follow the AGENTS.md § 2 rule:
announce in BROADCAST first.

**Lane assignment is unclaimed pending T-002.** The four slots in AGENTS.md § 1 are still `dev-1`…
`dev-4`; pick a lane at kickoff and fill in your handle there. The *decomposition* is what needs
ratifying here, not who takes which slot.
...
- **Engine + `ORDER BY`:**
  ```sql
  ENGINE = MergeTree
  PARTITION BY toYYYYMM(event_time)      -- 2 partitions over 5wk; day-partitioning is over-granular
  ORDER BY (event_time, app_id, geo_device_id, advertiser_id)
  ```
  **Justification:** *every* query in this product is time-bounded first — a window versus a
  same-weekday baseline window. Leading the sort key with `event_time` means both the incident
  window and the baseline windows are contiguous granule reads. Dimension columns follow in
  ascending cardinality-usefulness order for the secondary filter. We deliberately do **not** lead
  with a dimension: there is no single dominant slice, the drill-down sweeps all of them, and the
  rollup table (below) is what makes dimension access cheap.
...
- **Canonical DDL lives at:** `ingest/schema.sql` — **one file, Lane A owns it, everyone else reads
  it.** Any change needs `Breaking:` in the commit trailer plus a BROADCAST entry.
...
|---|---|---|---|
| `schema.sql` — table, dictionary and MV names + columns | Lane A | Lanes B, C | `ingest/schema.sql`. Lane B writes SQL against these names before any data is loaded. |
| `Evidence` — one computed number + its provenance | Lane B | Lanes C, D | `analysis/contracts.py`. `{id, label, value, unit, sql, sql_hash, window, filters}` |
| `Finding` — one conclusion, with its evidence | Lane B | Lanes C, D | `analysis/contracts.py`. `{channel, segment, metric, delta_abs, delta_pct, revenue_impact_usd, significance_sigma, status: found\|cleared, evidence_ids[]}` |
| `Investigation` — the full ordered result | Lane C | Lane D | `api/contracts.py`. `{request, plan_steps[], findings[], evidence[], ruled_out[], primary_channel, trace_id}` |
...
| R-008: Two lanes define "campaign" or "baseline" differently | Medium | Medium — inconsistent numbers across endpoints, reads as untrustworthy | D-007 and D-008 fix both once, in this file; detectors import from `analysis/contracts.py` rather than redefining | Lane B |
