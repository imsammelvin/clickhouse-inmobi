...
> de-shadows, it grounds every number." — the gaps have been the _front door_, not the engine.
...
**Trace, for "no trace, no credit":** every call = one OTel span + one JSONL line written _as it
happens_, `export_trace` for the artifact, OTel trace id in both so the file and ClickStack join.
...
- **Never widen an investigation window.** My first join merged overlapping windows and investigated
  ...
  lost entirely. Grouping must only change what the report _says_.
...
- **Rank on losses, not on movement.** Three of the top six windows by |move| were CTR _up_ 30-58%.
...
queries** — `find_incidents` alone 135M rows. Rows _returned_ are bounded by dimension cardinality and
that invariant holds; rows _read_ are undefended, ~38x the fact table per run. **This is the
