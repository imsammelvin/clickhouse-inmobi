...
> metric, `revenue` is money the platform _earns_ on impressions, `publisher_tier` is InMobi's own
...
**Why that costs real money:** in adtech the diagnosis delay _is_ the loss. A fill-rate break that
...
_rule things out_ makes this worse, not better.

### The insight we are building on

Revenue moves for a small number of structurally different reasons. Naming _where_ it moved is
...
| Revenue moves for six structurally different reasons. Naming _where_ it moved is table stakes.   |
...
| 4   | **Mix shift**       | every segment's rate is flat; weights moved toward cheaper inventory                 | **Nobody.** Nothing is broken    |
| 5   | **Seasonality**     | matches same-weekday trailing baseline within band                                   | **Nobody.** Suppress the alert   |
| 6   | **Exogenous event** | sharp onset _and_ offset, geo+category concentrated, no same-weekday precedent       | Planning — prep for the next one |
...
- **Dictionaries for the star schema.** `apps`, `advertisers`, `geo_device` are held in RAM;
  `dictGet` replaces a three-way JOIN on _every_ drill-down query. Idiomatic, and it matters here
...
- **The "wow" moment:** the **ruled-out list, proven by the trace.** Anyone can show a chart going
  ...
  _"seasonality checked and cleared at 6.1σ; traffic mix explains only −$2k of the −$58k; render
  rate normal; four other regions within band"_ — and then opening the Langfuse trace live to show
...
- **Beat-by-beat:**
  ...
  3. **The ruled-out block.** Walk it. Then run the **seasonality decoy** date and show the system
     return _"no anomaly — this is a weekend, within band"_ instead of crying wolf.
...
- Detection sweep across core metrics × time buckets — seasonality-aware baseline, two-gate flag
  (relative % **and** stddev)
- Revenue-identity walk (Requests → Fill rate → Impressions/Fills → eCPM) to find _which factor_ moved
...
- **Residualization — iterative deflation to separate causes from contamination** (D-017). After
  ...
  exceeds the band. Output is the _minimal_ set of true causes, plus every cleared segment with its
...
- **Rollup-backed queries so cost is independent of event volume.** Measured per stage by
  ...
  the whole window, and it grows with _cardinality × time_ rather than with events — so the same
...
- **Event fingerprinting** — computes the _shape_ of an exogenous event (sharp onset/offset, geo +
...
- Langfuse trace per investigation, every stage as a span — the trace _is_ a deliverable (§ 10)
...
- **Backfill capacity:** when demand exits a segment, which _other_ advertisers already bid on that
  same inventory, at what eCPM, and with how much unused headroom? This is what turns a diagnosis
  into a recovery plan — _"−$51k/day exposure; adv_0107 and adv_0455 bid the same inventory at
  within 8% of the lost eCPM and are taking 61% of available volume."_
...
- **Authentication, production deployment, alerting integrations** (PagerDuty etc.) — explicitly
  called out as not rewarded by the problem statement. We _detect_; we do not page.
...
| ClickStack observability         | Instruments the read workload: traces, metrics and logs per query pass                                                                               | ClickStack (HyperDX)          | Lane C (`MOHANSUNDAR K`)   | `observability/`, `main.ts`                                                |
...
**What exists as of 2026-08-01 T+4h** — the table above is the plan; this is the repo:

| Path                        | Landed in      | State                                                                                                                                                                       |
| --------------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `clickhouse/`, `scripts/`   | `bec2a35`      | Schema, dictionaries, `ad_events_enriched`, idempotent load. 9M rows live.                                                                                                  |
| `backend/`                  | `a299fe8`      | Investigation engine: detect → decompose → localize → residualize → classify → price. Plus `criteria.ts` (judging gate), `benchmark.ts` (T-042), `scan.ts`, `grounding.ts`. |
| `backend/langfuse/`         | `bbcc17c` (#6) | Tracing proof-of-concept. **Not yet wired to `orchestrate.ts`** — T-020 is `review`, not `done`.                                                                            |
| `observability/`, `main.ts` | `c4f2fd8` (#7) | ClickStack: traces, metrics and logs from the read workload.                                                                                                                |
| `api/server.ts`             | `c4f2fd8` (#7) | HTTP surface, early.                                                                                                                                                        |
| `mcp/`, `librechat/`        | —              | Not started.                                                                                                                                                                |

> Two seams to close, and both are integration rather than new building: `backend/langfuse/` traces
> a standalone pipeline, not the real `investigate()` — the `Ledger` already records a stage and a
> `runId` per query, so the spans have somewhere obvious to attach. And `observability/` instruments
> the read workload in `main.ts` rather than the investigation.
...
1. **`advertiser_id` is empty on unfilled requests.** Advertiser-sliced _fill rate_ is therefore
...
| Service latency/error telemetry                                                | Lane A (`backend/`, instrumented) | Lane C (`observability/`, `main.ts`)              | OTel/HTTP instrumentation emitted by `backend/`, ClickStack ingests                                                                                              |
...
> **Internal freeze note:** freeze pipeline _logic/thresholds_ 1–2 hours before the announced
...
- [ ] **Detection sweep finds the planted training-set anomalies** — including the pure-seasonality
      one, correctly _ruled out_ rather than alarmed on — with zero manual threshold tuning per
      incident. _(rubric: detection & localization accuracy, "avoid crying wolf")_
- [ ] **Every planted anomaly we find is named to a specific segment**, not a region-level hand-wave.
      _(rubric: "localized")_
- [ ] **Zero ungrounded numerals.** Every number in a narrated diagnosis is reproducible by
      ...
      over the full demo run. _(rubric: trustworthiness — one fabricated figure costs more than a
      miss)_
- [ ] **End-to-end (trigger → narrated diagnosis) in single-digit seconds** on the full 9M-row
      dataset; well under a minute in the worst case. _(rubric: "fast")_
- [ ] **≥90% of the analytical work happens in ClickHouse**, measured as: no stage exports more than
      1,000 rows to the backend. _(rubric: "analytical depth in ClickHouse")_
- [ ] **Every response carries a `traceId`** that opens a readable, ordered investigation in
      Langfuse. _(rubric: traceability — no trace, no credit)_
...
| D-005 | 2026-08-01 | The product is a **diagnosis engine with an API**; chat is one client of it, not the product                                                                                                                                                                  | Judges reward the investigation loop, not scaffolding. An API is testable and traceable in a way a chat UI is not. Compatible with D-003 — LibreChat stays the only _UI_.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | sam (proposed)                                 |
...
| R-003 | We cry wolf on the planted seasonality decoy specifically                                        | Medium     | High — explicitly in the rubric                                           | Ruleout battery runs _before_ narration; a finding that fails significance is reported as `cleared`, never dropped silently                                                    | Lane A        |
...
| R-007 | Unseen-incident dataset differs in shape from the sample                                         | Medium     | High — the highest-weighted deliverable                                   | Ingest is schema-driven and idempotent; dry-reload the _sample_ data through the full path at M3 to prove the loader is re-runnable                                            | Lane B        |
